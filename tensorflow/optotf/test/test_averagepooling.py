...
class TestAveragePooling1D(unittest.TestCase):
    ...
    def _test_1d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1, H=6, C=2):
        xr, xi = self._create_1d_inputs(dtype, N, H, C)
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(inputs)
            out_complex = optotf.averagepooling.averagepooling1d(inputs=inputs, pool_size=pool_size, strides=strides,
    ...
    def test_1d(self):
        ...
        # complex-valued
        padding_mode = 'VALID'
        self._test_1d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float32)

        self._test_1d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float64)
...
# to run execute: python -m unittest [-v] optotf.averagepooling
class TestAveragePooling2D(unittest.TestCase):
    ...
    def _test_2d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1, H=10, W=10, C=2, argmax=False):
        xr, xi = self._create_2d_inputs(dtype, N, H, W, C)
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=False) as tape:
            tape.watch(inputs)
            out_complex = optotf.averagepooling.averagepooling2d(inputs=inputs, pool_size=pool_size, strides=strides,
                                            alpha=alpha, beta=beta,
                                            mode=padding_mode, dilations_rate=dilations_rate)
            gradients=tape.gradient(tf.math.reduce_sum(out_complex), inputs)
        ...
        self.assertTrue(np.abs(np.array([N, padding_shape(H, pool_size[0], strides[0], dilations_rate[0], padding_mode),
                                         padding_shape(W, pool_size[1], strides[1], dilations_rate[1], padding_mode), C])
                               - np.array(out_complex.shape)).all() < 1e-8)
        self.assertTrue(np.abs(np.array(inputs.shape) - np.array(gradients.shape)).all() < 1e-8)
    ...
    def test_2d(self):
        ...
        # complex-valued
        padding_mode = 'VALID'
        self._test_2d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float32)

        self._test_2d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float64)
...
class TestAveragePooling3D(unittest.TestCase):
    ...
    def _test_3d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1, H=6, W=6, D=6, C=2, argmax=False):
        xr, xi = self._create_3d_inputs(dtype, N, H, W, D, C)
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(inputs)
            out_complex = optotf.averagepooling.averagepooling3d(inputs=inputs, pool_size=pool_size, strides=strides,
                                            alpha=alpha, beta=beta,
                                            mode=padding_mode, dilations_rate=dilations_rate)
            gradients = tape.gradient(tf.math.reduce_sum(out_complex), inputs)
        ...
        self.assertTrue(np.abs(np.array([N, padding_shape(H, pool_size[0], strides[0], dilations_rate[0], padding_mode),
                                         ...
                                         padding_shape(D, pool_size[2], strides[2], dilations_rate[2], padding_mode), C])
                               - np.array(out_complex.shape)).all() < 1e-8)
        self.assertTrue(np.abs(np.array(inputs.shape) - np.array(gradients.shape)).all() < 1e-8)
    ...
    def test_3d(self):
        ...
        # complex-valued
        padding_mode = 'VALID'
        self._test_3d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float32)

        self._test_3d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float64)
...
class TestAveragePooling4D(unittest.TestCase):
    ...
    def _test_4d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1,
                                   T=4,  H=6, W=6, D=6, C=2, argmax=False):
        xr, xi = self._create_4d_inputs(dtype, N, T, H, W, D, C)
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(inputs)
            out_complex = optotf.averagepooling.averagepooling4d(inputs=inputs, pool_size=pool_size, strides=strides,
                                                           alpha=alpha, beta=beta,
                                                           mode=padding_mode, dilations_rate=dilations_rate)
            gradients = tape.gradient(tf.math.reduce_sum(out_complex), inputs)
    ...
    def test_4d(self):
        ...
        # complex-valued
        padding_mode = 'VALID'
        self._test_4d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float32)

        self._test_4d_averagepooling_complex(pool_size, strides, alpha, beta, padding_mode, dilations_rate, np.float64)
