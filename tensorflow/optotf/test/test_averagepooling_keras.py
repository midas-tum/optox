...
class TestFunction1D(unittest.TestCase):
    ...
    def _test_1d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1,
                                    H=10,  C=2):
        xr, xi = self._create_1d_inputs(dtype, N, H, C)
        model = optotf.keras.averagepooling.Averagepooling1d(pool_size=pool_size, strides=strides, alpha=alpha, beta=beta,
...
class TestFunction2D(unittest.TestCase):
    ...
    def _test_2d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1,
                                    H=10, W=10, C=2):
        xr, xi = self._create_2d_inputs(dtype, N, H, W, C)
        model = optotf.keras.averagepooling.Averagepooling2d(pool_size=pool_size, strides=strides, alpha=alpha,
                                                             beta=beta, mode=padding_mode, dilations_rate=dilations_rate)
        # test type tf.complex, padding
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(inputs)
...
class TestFunction3D(unittest.TestCase):
    ...
    def _test_3d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1,
                                    H=10, W=10, D=6, C=2):
        xr, xi = self._create_3d_inputs(dtype, N, H, W, D, C)
        model = optotf.keras.averagepooling.Averagepooling3d(pool_size=pool_size, strides=strides, alpha=alpha, beta=beta,
                                 mode=padding_mode, dilations_rate=dilations_rate)
        # test type tf.complex, padding
        inputs = tf.complex(xr, xi)
        with tf.GradientTape(persistent=True) as tape:
...
class TestFunction4D(unittest.TestCase):
    ...
    def _test_4d_averagepooling_complex(self, pool_size, strides, alpha, beta, padding_mode, dilations_rate, dtype, N=1,
        ...
        model = optotf.keras.averagepooling.Averagepooling4d(pool_size=pool_size, strides=strides, alpha=alpha, beta=beta,
                                 mode=padding_mode, dilations_rate=dilations_rate)
