...
__all__ = ['averagepooling1d', 'averagepooling1d_grad_backward', 'averagepooling2d', 'averagepooling2d_grad_backward',
           'averagepooling3d', 'averagepooling3d_grad_backward', 'averagepooling4d', 'averagepooling4d_grad_backward']
...
_ext = tf.load_op_library(tf.compat.v1.resource_loader.get_path_to_datafile("tf_averagepooling_operator.so"))


def averagepooling1d(inputs, pool_size=(2,), strides=(2,), alpha=1, beta=1, channel_first=False, name="averagepooling_1d",
                 dilations_rate=(1,), pad=(0, ), mode='VALID', ceil_mode=True):
    op = _ext.averagepooling1d
    ...
    out_real, out_imag = op(input_real, input_imag,
...
@_ops.RegisterGradient("Averagepooling1d")
def grad(op, grad1, grad2):
    op_backward = _ext.averagepooling1d_grad_backward
...
def averagepooling2d(inputs, pool_size=(2, 2), strides=(2, 2), alpha=1, beta=1, channel_first=False, name="averagepooling_2d",
                 dilations_rate=(1, 1), pad=(0,0), mode='VALID', ceil_mode=True):
    op = _ext.averagepooling2d
    ceil_mode = int(ceil_mode)
    if type(inputs) is list:
        inputs = tf.complex(inputs[0], inputs[1])
    if inputs.dtype.is_complex is False:
...
@_ops.RegisterGradient("Averagepooling2d")
def grad(op, grad1, grad2):
    grad2 = grad1
    op_backward = _ext.averagepooling2d_grad_backward
    top_diff_real, top_diff_imag = tf.math.real(grad1), tf.math.imag(grad2)
    kernel_h = op.get_attr("kernel_h")
...
def averagepooling3d(inputs, pool_size=(2, 2, 2), strides=(2, 2, 2), alpha=1, beta=1, channel_first=False,
                 name="averagepooling_3d",
                 dilations_rate=(1, 1, 1), pad=(0,0,0), mode='VALID', ceil_mode=True):
    op = _ext.averagepooling3d
...
@_ops.RegisterGradient("Averagepooling3d")
def grad(op, grad1, grad2):
    grad2 = grad1
    op_backward = _ext.averagepooling3d_grad_backward
    top_diff_real, top_diff_imag = tf.math.real(grad1), tf.math.imag(grad2)
...
def averagepooling4d(inputs, pool_size=(2, 2, 2, 2), strides=(2, 2, 2, 2), alpha=1, beta=1, channel_first=False,
                 name='averagepooling_4d', dilations_rate=(1, 1, 1, 1), pad=(0, 0, 0, 0), mode='VALID', ceil_mode=True):
    ceil_mode = 1 if ceil_mode == True else 0
...
@_ops.RegisterGradient("Averagepooling4d")
def grad(op, grad1, grad2):
    op_backward = _ext.averagepooling4d_grad_backward
    top_diff_real, top_diff_imag = tf.math.real(grad1), tf.math.imag(grad2)
