...
class TestMaxpooling2d(unittest.TestCase):
    def _run_test(self, dtype, padding_mode):
        ...
        op = optopy.maxpooling.Maxpooling2d(pool_size, pads, strides, dilations_rate, alpha, beta, padding_mode, dtype, argmax=True)
        out_complex, out_idx = op.forward(x)
        ...
        back_out_indices = op.backward(x, out_complex, x_diff, indices=out_idx, with_indices=True)
...
class TestMaxpooling3d(unittest.TestCase):
    def _run_test(self, dtype, padding_mode):
        ...
        op = optopy.maxpooling.Maxpooling3d(pool_size, pads, strides, dilations_rate, alpha, beta, padding_mode, dtype, argmax=True, ceil_mode=True)
        out_complex, out_idx = op.forward(x)
        self.assertTrue(np.abs(np.array([N, padding_shape(H, pool_size[0], strides[0], dilations_rate[0], padding_mode),
                                         padding_shape(W, pool_size[1], strides[1], dilations_rate[1], padding_mode),
