import numpy as np
import _ext.py_maxpooling_operator

__all__ = ['float_2d', 'double_2d', 'float_3d', 'double_3d', 'float_4d', 'double_4d']

float_1d = _ext.py_maxpooling_operator.MaxPooling1d_float
double_1d = _ext.py_maxpooling_operator.MaxPooling1d_double

float_2d = _ext.py_maxpooling_operator.MaxPooling2d_float
double_2d = _ext.py_maxpooling_operator.MaxPooling2d_double

float_3d = _ext.py_maxpooling_operator.MaxPooling3d_float
double_3d = _ext.py_maxpooling_operator.MaxPooling3d_double

float_4d = _ext.py_maxpooling_operator.MaxPooling4d_float
double_4d = _ext.py_maxpooling_operator.MaxPooling4d_double
    ...
    if change_back == False:
        for x in inputs:
            ...
            if x.ndim == 3 and dimensions == '2d':
                ...
                raise Exception('input dimension should be 3, 4, 5 or 6!')
            outputs.append(x)
    else:
        for x in inputs:
            if x.ndim == 2:
                # (h,c)->(c,h)
                x = np.transpose(x, (1, 0))
            elif x.ndim == 2 and dimensions == '1d':
                # (b,h,c)->(b,c,h)
                x = np.transpose(x, (0, 2, 1))
            if x.ndim == 3 and dimensions == '2d':
                # (h,w,c)->(c,h,w)
    ...
    if len(shape_in) == 2:
        ...
        if dtype == torch.float32:
            return float_1d(shape_in[0], pool_size[0], strides[0],
                            ...
                            int(with_indices), int(ceil_mode), mode)
        elif dtype == torch.float64:
            return double_2d(shape_in[0], pool_size[0], strides[0],
                             channels, alpha, beta, pads[0], dilations_rate[0], batch,
                             int(with_indices), int(ceil_mode), mode)
    elif len(shape_in) == 3:
        batch = shape_in[0]
        if dtype == torch.float32:
            ...
            return double_1d(shape_in[1], shape_in[2], pool_size[0], strides[0],
                             channels, alpha, beta, pads[0], dilations_rate[0], batch,
                             int(with_indices), int(ceil_mode), mode)
    ...
    if len(shape_in) == 5:
        ...
        if dtype == torch.float32:
            return float_4d(shape_in[0], shape_in[1], shape_in[2], shape_in[3],
                            ...
                            batch, int(with_indices),int(ceil_mode), mode)
        elif dtype == torch.float64:
...
class Maxpooling1d(object):
    """Max-pooling of a 1d tensor.

    This function downsamples a 1d tensor (rank 3) according their max values. the compared value is defined by
    ...
    Args:
        inputs: A 4D `Tensor`.
        pool_size: tuple of 1 integer, window size over which to take the maximum.
    ...
    def __init__(self, pool_size=(1, ), pads=(0, ), strides=(1, ), dilations_rate=(1, ), alpha=1, beta=1,
                 padding_mode='SAME', dtype=np.float32, channel_first=False, with_indices=False, argmax=False,ceil_mode=True):
    ...
    def forward(self, x_input):
        ...
        op = get_operator_1d(self.dtype, x_input.shape, pool_size=self.pool_size, strides=self.strides,
        ...
        if np.iscomplexobj(x_input):
            ...
            out_real, out_imag, out_idx = op.forward(x_real, x_imag)
            if self.channel_first:
                [out_real, out_imag, out_idx] = transpose_channel_first_last([out_real, out_imag, out_idx],
                                                                             change_back=True,
                                                                             dimensions='1d')
            if self.argmax:
                return out_real + 1j * out_imag, out_idx
            else:
                return out_real + 1j * out_imag

        else:
            out_real, out_idx = op.forward(x_input, x_input * 0)
            if self.channel_first:
                [out_real, out_idx] = transpose_channel_first_last([out_real, out_idx], change_back=True,
                                                                   dimensions='1d')
            if self.argmax:
                return out_real, out_idx
            else:
                return out_real
...
class Maxpooling2d(object):
    ...
    This function downsamples a 2d tensor (rank 4) according their max values. the compared value is defined by
    value=alpha*real_value^2+beta*imag_value^2.
    ...
    def __init__(self, pool_size=(1, 1), pads=(0, 0), strides=(1, 1), dilations_rate=(1, 1), alpha=1, beta=1,
                 padding_mode='SAME', dtype=np.float32, channel_first=False, with_indices=False, argmax=False,ceil_mode=True):
...
class Maxpooling3d(object):
    ...
    This function downsamples a 3d tensor (rank 5) according their max values. the compared value is defined by
    value=alpha*real_value^2+beta*imag_value^2.
    ...
    def __init__(self, pool_size=(1, 1, 1), pads=(0, 0, 0), strides=(1, 1, 1), dilations_rate=(1, 1, 1), alpha=1,
                 beta=1, padding_mode='SAME', dtype=np.float32, channel_first=False, with_indices=False, argmax=False,ceil_mode=True):
...
class Maxpooling4d(object):
    """Max-pooling of a 4d tensor.
    ...
    def forward(self, x_input):
        ...
        if np.iscomplexobj(x_input):
            ...
            out_real, out_imag, out_idx, time_out, height_out, width_out, depth_out = op.forward(x_real, x_imag)
            ...
            out_real, out_imag, out_idx = np.reshape(out_real, size_out), np.reshape(out_imag, size_out), np.reshape(
                out_idx, size_out)
            ...
            out_real, _, out_idx, time_out, height_out, width_out, depth_out = op.forward(x_input, x_input * 0)
            ...
            out_real, out_idx = np.reshape(out_real, size_out), np.reshape(out_idx, size_out)
