///@file py_gpunufft_operator.cpp
///@brief Python wrappers for gpuNUFFT operator
...
template<typename T>
py::array forward(optox::GPUNufftOperator<T> &op, py::array np_img, py::array np_csm, py::array np_trajectory, py::array np_dcf)
{
    ...
    auto csm = getComplexDTensorNp<complex_type, 3>(np_csm);
    ...
    rawdata_shape[1] = csm->size()[0]; // add number of channels
    rawdata_shape[2] = trajectory->size()[2];

    optox::DTensor<complex_type, 3> rawdata(rawdata_shape);

    op.forward({&rawdata}, {img.get(), csm.get(), trajectory.get(), dcf.get()});
    return dComplexTensorToNp(rawdata);
}

template<typename T>
py::array adjoint(optox::GPUNufftOperator<T> &op, py::array np_rawdata, py::array np_csm, py::array np_trajectory, py::array np_dcf)
{
    typedef typename optox::type_trait<T>::complex_type complex_type;
    typedef typename optox::type_trait<T>::real_type real_type;
    ...
    op.adjoint({&img}, {rawdata.get(), csm.get(), trajectory.get(), dcf.get()});
...
void declare_op(py::module &m, const std::string &typestr)
{
    ...
    // add pickle support
    .def("__getstate__", [](const optox::GPUNufftOperator<T> &op) {
        /* Return a tuple that fully encodes the state of the object */
        return py::make_tuple(op.getImgDim(), op.getOsf(), op.getKernelWidth(), op.getSectorWidth());
    })
    .def("__setstate__", [](optox::GPUNufftOperator<T> &op, py::tuple t) {
        if (t.size() != 4)
            throw std::runtime_error("Invalid state!");
        /* Invoke the constructor (need to use in-place version) */
        new (&op) optox::GPUNufftOperator<T>(t[0].cast<int>(), t[1].cast<T>(), t[2].cast<int>(), t[3].cast<int>());
    });
}

PYBIND11_MODULE(py_gpunufft_operator, m)
{
