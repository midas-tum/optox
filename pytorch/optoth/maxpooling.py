...
import _ext.th_maxpooling_operator

float_1d = _ext.th_maxpooling_operator.MaxPooling1d_float
double_1d = _ext.th_maxpooling_operator.MaxPooling1d_double

float_2d = _ext.th_maxpooling_operator.MaxPooling2d_float
double_2d = _ext.th_maxpooling_operator.MaxPooling2d_double

float_3d = _ext.th_maxpooling_operator.MaxPooling3d_float
double_3d = _ext.th_maxpooling_operator.MaxPooling3d_double

float_4d = _ext.th_maxpooling_operator.MaxPooling4d_float
double_4d = _ext.th_maxpooling_operator.MaxPooling4d_double
...
class Maxpooling1dFunction(torch.autograd.Function):
    ...
    Backward:
    Find backward max-pooling gradient of a 1d tensor.
    Only the gradient value with max-pooled indices can be transferred to the bottom.
    Args:
        ctx: A context object storing information from the forward path:
            x_input: A 3D `Tensor`.
            x_output:  A 3D `Tensor` outputs from maxpooling1d.
        grad_out: Backward flowing gradient as a 3D `Tensor`.
    Returns:
        bottom_diff : A 3D `Tensor` gradient from bottom (x_input side)
    ...
    def forward(ctx, x_input, pool_size, pads, strides, dilations_rate,
        ...
        if x_input.dtype == torch.complex64 or x_input.dtype == torch.complex128:
            ...
            out_real, out_imag, out_idx = ctx.op.forward(x_real, x_imag)
            if channel_first:
            ...
            out_real, _, out_idx = ctx.op.forward(x_real, x_real * 0)
            if channel_first:
    ...
    def backward(ctx, grad_out):
        ...
        bottom_diff_real, bottom_diff_imag = ctx.op.adjoint(inputs, inputs,
                                                            inputs, inputs,
                                                            top_diff_real, top_diff_imag, out_idx)
        if channel_first:
...
class Maxpooling2dFunction(torch.autograd.Function):
    ...
    Backward:
    Find backward max-pooling gradient of a 2d tensor.
    Only the gradient value with max-pooled indices can be transferred to the bottom.
    Args:
        ctx: A context object storing information from the forward path:
            x_input: A 4D `Tensor`.
            x_output:  A 4D `Tensor` outputs from maxpooling2d.
        grad_out: Backward flowing gradient as a 4D `Tensor`.
    Returns:
        bottom_diff : A 4D `Tensor` gradient from bottom (x_input side)
        the type is same as x_input.
    """
    @staticmethod
    def forward(ctx, x_input, pool_size, pads, strides, dilations_rate,
...
class Maxpooling3dFunction(torch.autograd.Function):
    ...
    Backward:
    Find backward max-pooling gradient of a 3d tensor.
    Only the gradient value with max-pooled indices can be transferred to the bottom.
    Args:
        ctx: A context object storing information from the forward path:
            x_input: A 5D `Tensor`.
            x_output:  A 5D `Tensor` outputs from maxpooling3d.
        grad_out: Backward flowing gradient as a 5D `Tensor`.
    Returns:
        bottom_diff : A 5D `Tensor` gradient from bottom (x_input side)
        the type is same as x_input.
    """
    @staticmethod
    def forward(ctx, x_input, pool_size, pads, strides, dilations_rate,
...
class Maxpooling4dFunction(torch.autograd.Function):
    ...
    Returns:
        A  complex `Tensor`which has the same type as x_input `tensor`
    Backward:
    Find backward max-pooling gradient of a 4d tensor.
    Only the gradient value with max-pooled indices can be transferred to the bottom.
    Args:
        ctx: A context object storing information from the forward path:
            x_input: A 6D `Tensor`.
            x_output:  A 6D `Tensor` outputs from maxpooling4d.
        grad_out: Backward flowing gradient as a 6D `Tensor`.
    Returns:
        bottom_diff : A 6D `Tensor` gradient from bottom (x_input side)
        the type is same as x_input.
    """
    @staticmethod
    def forward(ctx, x_input, pool_size, pads, strides, dilations_rate,
        ...
        if x_input.dtype == torch.complex64 or x_input.dtype == torch.complex128:
            ...
            out_real, out_imag, out_idx, time_out, height_out, width_out, depth_out = ctx.op.forward(x_real, x_imag)
            ctx.save_for_backward(out_idx)
            ...
            out_real, _, out_idx, time_out, height_out, width_out, depth_out = ctx.op.forward(x_real, x_real * 0)
            ctx.save_for_backward(out_idx)
            if len(x_real_shape) == 5:
                size_out = (time_out, height_out, width_out, depth_out, x_real_shape[-1])
            elif len(x_real_shape) == 6:
    ...
    def backward(ctx, grad_out):
        ...
        out_idx_shape = out_idx.shape
        size_out = (int(np.prod(np.array(out_idx_shape))),)
