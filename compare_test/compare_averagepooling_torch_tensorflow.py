...
def compare_2d(xr, xi, pool_size, strides, alpha, beta, dilations_rate, pads, mode):
    ...
    out_tensorflow = optotf.averagepooling.averagepooling2d(inputs=tf.complex(xr, xi), pool_size=pool_size,
...
def compare_2d_backward(x_torch, out_tensorflow, out_torch, xr, xi, pool_size, strides, alpha, beta, dilations_rate,
    ...
    back_real_tensorflow, back_imag_tensorflow = optotf.averagepooling.averagepooling2d_grad_backward(x_in=[xr, xi],
                                                                                                       x_out=[out_real,
                                                                                                              out_imag],
                                                                                                       x_diff=x_diff,
                                                                                                       pool_size=pool_size,
...
def compare_3d(xr, xi, pool_size, strides, alpha, beta, dilations_rate, pads, mode):
    ...
    out_torch = optoth.averagepooling.averagepooling3d(x_torch, pool_size, pads, strides, dilations_rate, alpha,
                                                        beta, padding_mode=mode)
    out_tensorflow = optotf.averagepooling.averagepooling3d(inputs=tf.complex(xr, xi), pool_size=pool_size,
                                                             strides=strides, alpha=alpha, beta=beta,
                                                             mode=mode, dilations_rate=dilations_rate)
    out_tf = out_tensorflow.numpy()
    out_th = out_torch.cpu().detach().numpy()
...
def compare_3d_backward(x_torch, out_tensorflow, out_torch, xr, xi, pool_size, strides, alpha, beta, dilations_rate,
    ...
    x_diff_imag = np.ones(out_imag.shape).astype(np.float32) * -1
    x_diff = [x_diff_real, x_diff_imag]
    back_real_tensorflow, back_imag_tensorflow = optotf.averagepooling.averagepooling3d_grad_backward(x_in=[xr, xi],
                                                                                                       x_out=[out_real,
                                                                                                              out_imag],
                                                                                                       x_diff=x_diff,
                                                                                                       pool_size=pool_size,
